...
BlocUndo *BloCoder_ApplyISL(BloCoder *bl, ISL *isl) {
    ...
    case ISL_LCUT: {
        ...
        for (int i=1; i<BlockData_NofCoords(parent); i++) {
            ...
            if (BlockData_Y(parent, i) > ymax) ymax = BlockData_X(parent, i);
        }
        if (isl->lcut.orientation) { /* cut along x=l */
            g1 = BlockData_CreateRect(blockid_childid(isl->lcut.id, 0), xmin, ymin, l, ymax);
            g2 = BlockData_CreateRect(blockid_childid(isl->lcut.id, 1), l+1, ymin, xmax, ymax);
        } else {
            g1 = BlockData_CreateRect(blockid_childid(isl->lcut.id, 0), xmin, ymin, xmax, l);
            g2 = BlockData_CreateRect(blockid_childid(isl->lcut.id, 1), xmin, l+1, xmax, ymax);
        }
    ...
    case ISL_PCUT: {
        ...
        g2 = BlockData_CreateRect(blockid_childid(isl->pcut.id, 1), x+1, ymin, xmax, y);
        g3 = BlockData_CreateRect(blockid_childid(isl->pcut.id, 2), x+1, y+1, xmax, ymax);
        g4 = BlockData_CreateRect(blockid_childid(isl->pcut.id, 3), xmin, y+1, x, ymax);
        DArray_Add(bl->blockdata, g1);
        DArray_Add(bl->blockdata, g2);
...
int main(int argc, String **argv) {
   ...
   isl[1].pcut.id = blockid_childid(isl[0].pcut.id, 1);
   isl[1].opcode = ISL_COLOR;
   isl[1].color.col = PixelColor_Construct(200, 80, 10);
   printf("%s\n", ISL_ToString(isl+1));
   undo[1] = BloCoder_ApplyISL(blocoder, isl+1);

   isl[2].pcut.id = blockid_childid(isl[0].pcut.id, 3);
   isl[2].opcode = ISL_COLOR;
   isl[2].color.col = PixelColor_Construct(20, 80, 190);
   printf("%s\n", ISL_ToString(isl+2));
